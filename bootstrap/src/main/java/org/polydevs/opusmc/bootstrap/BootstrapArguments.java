...
final class BootstrapArguments {
    ...
    static BootstrapArguments parse(String[] arguments) {
        ...
        for (int index = 0; index < arguments.length; index++) {
            ...
            if ("--opus-game-main".equals(argument)) {
                gameMain = requireValue(arguments, ++index, argument);
            } else if ("--opus-game-classpath-file".equals(argument)) {
                classpathFile = Paths.get(requireValue(arguments, ++index, argument));
            } else if ("--opus-game-arguments-stdin".equals(argument)) {
        ...
        if (gameMain == null || gameMain.trim().isEmpty()) {
            throw new IllegalArgumentException("Missing --opus-game-main");
        }
        if (classpathFile == null) {
            throw new IllegalArgumentException("Missing --opus-game-classpath-file");
        }
        if (!readsGameArgumentsFromStdin) {
