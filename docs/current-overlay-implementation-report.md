...
OPUS chạy Minecraft Java Edition 1.8.9 bằng Forge 11.15.1.2318 cùng OptiFine HD U M5. Overlay in-game không được render bởi launcher React/Tauri. Nó là một Forge client-side mod Java 8 được Forge nạp vào class loader của game.
...
Tuy nhiên OpusClientScreen hiện tại là prototype bị từ chối. Nó là một settings form duy nhất, không phải HUD editor, Mod Hub hay module-detail flow theo UI tham chiếu. Nó không được coi là nền UI sản phẩm.
...
Client mod là opus-forge-client-0.0.1-preview.3.jar. Nó là FML mod bình thường, không có FMLCorePlugin trong manifest. Gradle kiểm tra điều này sau reobfuscation tại [build.gradle](/Users/zvwgvx/Project/opus-client/game/client-mod/build.gradle:127).

## 3. Chuỗi launch

1. Tauri/Rust xác nhận Forge, OptiFine, Java và artifact OPUS theo hash lock.
2. Runtime stage OptiFine, coremod cũ và typed client mod vào thư mục mods riêng của profile.
3. Java được chạy qua org.polydevs.opusmc.bootstrap.ForgeBootstrapMain.
4. Forge nạp mod opusclient và gọi event lifecycle của mod.
5. Client mod đăng ký keybind, Forge event listener và HUD renderer.

Launch mode và việc stage artifact hiện ở [launch.rs](/Users/zvwgvx/Project/opus-client/launcher/opus-runtime/src/launch.rs:107) và [launch.rs](/Users/zvwgvx/Project/opus-client/launcher/opus-runtime/src/launch.rs:388).

Hai product bundle là:

- OPUS Client.app: Premium launcher.
- OPUS Client Demo.app: offline/demo launcher, có offline username và data root riêng.

## 4. In-game overlay implementation

### 4.1 Entry point

OpusClientMod là FML entry point, clientSideOnly = true; trong lifecycle nó tạo ClientOverlayController và đăng ký hook. Xem [OpusClientMod.java](/Users/zvwgvx/Project/opus-client/game/client-mod/src/main/java/org/polydevs/opusmc/client/OpusClientMod.java:15).
...
Right Shift và Client Options cùng gọi openOptions(), rồi mở new OpusClientScreen(this). Xem [ClientOverlayController.java](/Users/zvwgvx/Project/opus-client/game/client-mod/src/main/java/org/polydevs/opusmc/client/ClientOverlayController.java:35).
...
OpusClientScreen kế thừa GuiScreen, không pause game và vẽ thủ công bằng drawRect, Minecraft FontRenderer và texture OPUS. Controls là subclass GuiButton.
...
Mã nằm ở [OpusClientScreen.java](/Users/zvwgvx/Project/opus-client/game/client-mod/src/main/java/org/polydevs/opusmc/client/OpusClientScreen.java:17).
...
Khi launch, launcher thêm JVM argument:
    ...
    -Dopus.utility.settings.file=<absolute path>/utility-settings-v1.json

Xem [launch.rs](/Users/zvwgvx/Project/opus-client/launcher/opus-runtime/src/launch.rs:313) và [lib.rs](/Users/zvwgvx/Project/opus-client/desktop/src-tauri/src/lib.rs:1146).

Trong game, UtilitySettingsStore đọc cùng file bằng Gson. Chỉ key utilities.fps được typed client mod dùng/update; key khác chỉ được preserve trong JSON. Ghi file dùng temporary file + atomic move. Xem [UtilitySettingsStore.java](/Users/zvwgvx/Project/opus-client/game/client-mod/src/main/java/org/polydevs/opusmc/client/UtilitySettingsStore.java:21).

Hiện chưa có IPC hai chiều live; launcher và game đồng bộ qua file.

## 6. Rendering và blur

Hiện không có blur thật. OpusClientScreen chỉ vẽ scrim alpha đen. Không có framebuffer capture, downsample, FBO, shader pass, shader reload hoặc OptiFine compatibility layer.

Vì vậy UI hiện tại không đáp ứng yêu cầu background blur.

## 7. Legacy coremod: tình trạng và rủi ro

Repository có coremod/ASM cũ. Typed Forge mod mới không dùng nó để render UI, nhưng ForgeBootstrap vẫn stage opus-forge-coremod.jar cùng typed client mod vì contract bootstrap hiện yêu cầu nó.

Demo và UI-preview thêm -Dopus.legacy.ui.disabled=true để UI legacy không chạy. Premium hiện không tự thêm cờ này vì code dùng feature guard qa-edition hoặc ui-preview.
...
- Rust test opus-runtime: 35 tests pass;
- Rust test opus-desktop qa-edition: 11 tests pass;
- TypeScript check.

Smoke launch Demo sau khi sửa access modifier ghi nhận:

    OPUS client options registered: Right Shift and the pause-menu Client Options button open the real utility surface.
    OPUS Forge client module loaded; the typed Client Options surface is ready.
    game.status = running
