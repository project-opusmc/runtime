...
Everything under `game/` produces Java 8 bytecode for RBW's pinned Forge +
OptiFine 1.8.9 runtime. Forge owns Minecraft classloading. RBW joins that
...
| `forge` | `bootstrap`, `core`, `patches`, Forge/LaunchWrapper at compile time | Packages `rbw-forge-coremod-*.jar` with the `FMLCorePlugin` manifest and Forge transformer adapter. |
...
- `bootstrap-*.jar`; and
- `rbw-forge-coremod-*.jar`; and
- `rbw-forge-client-*.jar`.

The managed runtime supplies Forge, LaunchWrapper, and Forge's ASM 5.0.3. They
must not be bundled into either RBW Forge artifact. `scripts/prepare-desktop-assets.sh`
copies all three RBW artifacts into the Tauri resources.
...
2. The JVM starts `dev.rbw.bootstrap.ForgeBootstrapMain`. Its ordinary game
...
4. Forge LaunchWrapper creates its `LaunchClassLoader`, applies the Forge and
   OptiFine tweakers, and discovers the RBW coremod plus normal client mod from
   the isolated `game/mods/` directory.
5. The coremod manifest registers `dev.rbw.forge.RbwLoadingPlugin` as an
   `IFMLLoadingPlugin`. Its sorting index places RBW's verified obfuscated-name
   patches before Forge's deobfuscation transformer while preserving OptiFine's
   earlier tweaker ordering.
6. `RbwLoadingPlugin` starts the local telemetry lifecycle and registers
   `RbwForgeClassTransformer`, an `IClassTransformer` adapter. The adapter
   maps Forge's raw/deobfuscated class names back to the locked 1.8.9 targets
   and forwards only those targets through RBW's deterministic patch chain.
7. The first-frame lifecycle patch marks the RBW core as running. Launcher and
...
Minecraft classes. `RbwForgeClassTransformer` implements Forge's
`IClassTransformer` contract; it neither creates nor delegates to RBW's former
child-first game loader.

The adapter deliberately restricts transformation to the known 1.8.9 classes
and the RBW options-screen placeholder. It handles both raw obfuscated names
...
| --- | --- | --- | --- |
| 100 | `rbw.window-title` | `ave` | Replaces the locked window-title anchor. |
| 105 | `rbw.forge-lifecycle` | `ave` | Marks the telemetry lifecycle running at the verified frame-loop entry. |
| 110 | `rbw.minecraft-telemetry` | `ave` | Frame-loop and tick timing hooks. |
| 112 | `rbw.client-options` | `ave`, `axp`, `avo` | Right-Shift entry, pause-menu button, HUD hook, and Utilities screen entry. |
| 115 | `rbw.render-telemetry` | `bfk` | Render-scope timing. |
| 120 | `rbw.combat-telemetry` | `bda`, `bcy` | Attack intent, queued attack packet, and entity-status correlation. |
| 130 | `rbw.network-telemetry` | `ek` | Inbound/outbound packet counters and connection lifecycle. |

Look for `[RBW/FORGE] coremod registered`, `[RBW/FORGE] transforming`, and
`[RBW/PATCH] applied …` in the per-session game output when diagnosing a Forge
...
`ClientUiHooks` remains the boundary between injected bytecode and RBW UI code.
...
| --- | --- | --- |
| `rbw.game.statusFile` | macOS launch path | Lifecycle status file read by the launcher. |
| `rbw.diagnostics.file` | every real launch | Local JSONL telemetry destination. |
| `rbw.utility.settings.file` | desktop launcher | HUD utility preferences. |
| `rbw.brand.wordmark.file` | desktop launcher | Branding image for the in-game UI. |
...
When adding or changing an RBW patch:

1. Update the explicit chain and `RbwForgeClassTransformer` target mapping.
