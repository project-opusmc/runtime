...
final class OpusPerformanceDetailPage extends OpusPanelPage {
    ...
    OpusPerformanceDetailPage(ClientOverlayController controller, UiRuntime runtime) {
        super(controller, runtime);
        back = button("BACK", UiActionButton.Tone.NEUTRAL, new Runnable() {
            @Override
            public void run() {
                OpusPerformanceDetailPage.this.runtime.navigate(UiRoute.modHub());
            }
        });
        enabled = button("", UiActionButton.Tone.NEUTRAL, new Runnable() {
            @Override
            public void run() {
                PerformanceOverlaySettings current = OpusPerformanceDetailPage.this.controller.performanceOverlay();
                OpusPerformanceDetailPage.this.controller.updatePerformanceOverlay(current.withEnabled(!current.enabled()));
                refreshLabels();
            }
        });
        anchor = button("", UiActionButton.Tone.NEUTRAL, new Runnable() {
            ...
            public void run() {
                PerformanceOverlaySettings current = OpusPerformanceDetailPage.this.controller.performanceOverlay();
                OpusPerformanceDetailPage.this.controller.updatePerformanceOverlay(current.withAnchor(current.anchor().next()));
                refreshLabels();
            }
        });
