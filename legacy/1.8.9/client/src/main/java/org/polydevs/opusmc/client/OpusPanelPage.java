...
abstract class OpusPanelPage extends OpusUiPage {
    ...
    OpusPanelPage(ClientOverlayController controller, UiRuntime runtime) {
        super(controller, runtime);
        close = button("X", UiActionButton.Tone.QUIET, new Runnable() {
            @Override
            public void run() {
                OpusPanelPage.this.runtime.requestClose();
            }
        });
    }
